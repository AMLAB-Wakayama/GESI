...
%
%
function [Result, GESIparam,SndInfo] = GESIv148(SndRef, SndTest, GCparam, GESIparam)
...
if isfield(GESIparam,'Sim') == 0 || isfield(GESIparam.Sim,'PowerRatio') ==0  % To control from the main
    ...
    GESIparam.Sim.PowerRatio = 0.55; % default 18 Nov 24
    disp('GESIparam.Sim.PowerRatio is set to 0.55 (default) -- OK? Return to continue > ')
    pause
...
if LenAbvThr > 0 %  to avoid division by zero   3 May 2025
    EfficiencyVal = (length(MeanEpTest)/LenAbvThr).^( GESIparam.Sim.weightAbvThrCmpnst + eps );  
    % GESIparam.Sim.weightAbvThrCmpnst == "eta" in Paper,    eps: to avoid  (anyval)^0 == 1;
else
    EfficiencyVal = 0;
end
weightAbvThrTest = EfficiencyVal*weightAbvThrTest0;
...
% -------------------------------
% Modified for multiple nrPwr for large simulation
% -------------------------------
for nrPwr = 1:length(GESIparam.Sim.PowerRatio)
    for nch = 1:NumCh  % == GCparam.NumCh
        ...
        weightGCFB    = weightAbvThrTest(nch)*SSIparam.weight(nch,:); % 14 Nov 24,  using only EP above threshold
        weightGCFBinv = weightAbvThrTest(nch)*max(1 - SSIparam.weight(nch,:),0);  % 14 Nov 24, sing only EP above threshold
        ...
        for nMFB = 1:LenMFB
            ...
            rPwr = GESIparam.Sim.PowerRatio(nrPwr); % added by YA, v122 on 23 Oct 22
    ...
    % Results of GESI
    Result.GESI.CosSim(:,:,nrPwr)               = CosSim_Mtrx; 
    Result.GESI.CosSim_Raw(:,:,nrPwr)       = CosSim_RawMtrx; 
    Result.GESI.CosSim_InvSSI(:,:,nrPwr)    = CosSim_InvSSIMtrx; 
    Result.GESI.CosSim_InvSSIRaw(:,:,nrPwr) = CosSim_InvSSIRawMtrx; 
    Result.GESI.CosSim_MeanGCFB(:,nrPwr)    = mean(CosSim_Mtrx,1); % mean value across GCFB
    Result.GESI.CosSim_MeanMFB(:,nrPwr)     = mean(CosSim_Mtrx,2); % mean value across MFB
    Result.GESI.dVal(nrPwr)                 = mean(CosSim_Mtrx,[1 2]); % mean value across GCFB

    Result.GESI.Pcorrect(nrPwr) = Metric2Pcorrect_Sigmoid(Result.GESI.dVal(nrPwr),GESIparam.Sigmoid);
    %　Preliminary output of Pcorrect -- It does not represent acculate SI

end  % nrPwr = 1:length(GESIparam.Sim.PowerRatio)


Result.d.GESI = Result.GESI.dVal; % for compativility
Result.Pcorrect.GESI = Result.GESI.Pcorrect; % for compativility


% -------------------------------
% plot Result.dIntrm.CosSimMtrx
if GESIparam.SwPlot == 2 & length(GESIparam.Sim.PowerRatio) == 1
...
% Removed
    ...
    % Result.dIntrm.GESI(:,:,nrPwr) = CosSimMtrx;  % Original intermediate  representation for d
    % Result.d.GESI(:,nrPwr)        = mean(CosSimMtrx(~isnan(CosSimMtrx))); % average non-NaN values  29 Jul 22
